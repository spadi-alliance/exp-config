...
do
    ...
    for ip in $IP_LIST
    do
	CMD="$READ_SCR_BIN $ip up ./dat/scr.${ip}.dat"
	echo "$CMD"
	eval "$CMD"
	...
	nch=16 #`cat ./dat/scr_rate.${ip}.txt | wc -l`
	rate_list=`awk 'NR<=16{printf ("%s", NR==1? $1 : ", "$1 )}' ./dat/scr_rate.${ip}.txt`
	val='{'
	...
	nch=16 #`cat ./dat/scr_cur.${ip}.txt | wc -l`
	count_list=`awk 'NR<=16{printf ("%s", NR==1? $1 : ", "$1 )}' ./dat/scr_cur.${ip}.txt`
	val='{'
