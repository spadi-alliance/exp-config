...
do
    echo "start device Scaler ${runID}"
        tmux new-window -d -n Scaler${runID} -t scaler -- ./start_device.sh Scaler # --host-ip 192.168.2.100
    sleep 0.2
done
...
do                                                                                                                                   
    ...
    tmux new-window -d -n ScrSink${runID} -t sink -- ./start_device.sh FileSink --service-name ScrSink # --host-ip 192.168.2.100       
    sleep 0.2                                                                                                                        
done         
