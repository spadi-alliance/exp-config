...
#    echo "start device Sampler ${runID}"
    #./start_device.sh.in AmQStrTdcSampler
#    tmux new-window -d -n C${runID} -t sampler -- ./start_device.sh AmQStrTdcSampler --service-name CirasTdcSampler #--host-ip 192.168.2.51
...
do
    echo "start device SubTime Frame Builder ${runID}"
	tmux new-window -d -n STF${runID} -t stfb -- ./start_device.sh STFBuilder --service-name CirasSTFBuilder --host-ip 192.168.2.51
    sleep 0.1
done
...
ssh -X esys '(cd run ; ./run-ciras.sh)' && ssh -X nestdaq@e50server05 '(cd run ; ./run-sink.sh)'
