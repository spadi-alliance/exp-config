...
tmux new-session -d -t scrsink
for runID in {0..38}
do
    echo "start device SCRSink ${runID}"
    tmux new-window -d -n SCRSINK${runID} -t scrsink -- ./start_device.sh FileSink --service-name ScrSink #--host-ip 192.168.2.51
    sleep 0.2
done

tmux kill-window -t scrsink:0
xterm -geometry 80x15+500+500 -T ScrSink -e tmux a -t scrsink &
